...
public:
    ...
    {
        {
            ...
            // but the std container guarantees that if an exception is thrown, the container remains unchanged.
    ...
    TType pop_back() {
        // when the exception is thrown, RAII will ensure the mutex is unlocked by stack unwinding
        std::unique_lock<std::mutex> lock(mutex_);
        // blocking current thread until queue is not empty and then lock mutex
        // cv_.wait will automatically unlock the mutex while waiting and re-lock it when notified
        // !!condition_variable wait() may throw std::system_error if the mutex is not locked
